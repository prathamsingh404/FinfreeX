          ...
          <header className="flex items-center justify-between gap-3 px-3 h-11 border-b border-border bg-surface shrink-0">
            <div className="flex items-center gap-2.5 min-w-0">
              {showBack && (
                <button
                  onClick={() => router.back()}
                  className="p-1 rounded text-muted hover:text-foreground hover:bg-hover transition-colors cursor-pointer mr-1 shrink-0"
                  title="Go back"
                >
          ...
          <div className="mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-8 pt-20 lg:pt-10 pb-14">
            <header className="fade-up mb-6">
              {showBack && (
                <button
                  onClick={() => router.back()}
                  className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium text-soft bg-surface border border-border hover:border-border-strong hover:text-foreground transition-all cursor-pointer mb-3"
                >
