...
import { Card, SectionTitle, Badge, Btn, Change, ProgressBar, Sparkline, fmt } from '@/components/ui/kit'
import { getAllQuotes, getSparkline } from '@/lib/mockData'
...
const MOCK_NEWS = [
  { title: 'RBI holds repo rate steady; signals data-dependent stance for coming quarters', source: 'Mint', time: '2h ago', tag: 'Macro' },
  { title: 'IT majors rally as US enterprise deal pipeline shows early recovery signs', source: 'Moneycontrol', time: '4h ago', tag: 'IT' },
  { title: 'Banking stocks lead gains on improving credit growth and stable NPAs', source: 'Economic Times', time: '6h ago', tag: 'Banking' },
  { title: 'FIIs turn net buyers in cash market after three weeks of outflows', source: 'Bloomberg', time: '8h ago', tag: 'Flows' },
  { title: 'Auto sales beat estimates on festive demand and new EV launches', source: 'Business Standard', time: '10h ago', tag: 'Auto' },
  { title: 'Crude softens on demand worries, easing pressure on OMC margins', source: 'Reuters', time: '12h ago', tag: 'Energy' },
]
...
export default function IntelligencePage() {
  ...
  const trending = useMemo(
    () => [...quotes].sort((a, b) => Math.abs(b.changePct) - Math.abs(a.changePct)).slice(0, 8),
    [quotes],
  )
  ...
  const sentimentTone = (s: Sentiment) => (s === 'Bullish' ? 'emerald' : s === 'Bearish' ? 'coral' : 'amber') as const
    ...
    >
      <div className="flex gap-1 p-1 rounded-xl bg-white/5 border border-white/10 w-fit mb-6">
        {(['analyst', 'news', 'trending'] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-5 py-2 rounded-lg text-xs font-semibold uppercase tracking-wide transition-all ${tab === t ? 'bg-emerald text-[#04120C]' : 'text-soft hover:text-foreground'}`}
          >
            {t}
          </button>
        ))}
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card>
              ...
              <textarea
                ...
                className="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-sm text-foreground placeholder:text-muted outline-none focus:border-emerald-bright resize-none mb-4"
              />
              <div className="flex gap-3">
                <Btn onClick={() => run()} disabled={loading || !query.trim()} className="flex-1 justify-center">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {SUGGESTED.map((s) => (
                    <button
                      key={s.label}
                      onClick={() => run(s.query)}
                      className="p-3 rounded-xl bg-white/5 border border-white/10 text-left hover:border-emerald/40 hover:bg-emerald/5 transition-all"
                    >
            ...
            {loading && (
              <Card>
                <div className="flex items-center gap-3 text-soft text-sm">
                  <span className="w-2 h-2 rounded-full bg-emerald-bright ticker-live" />
                    ...
                    <ul className="space-y-2">
                      {analysis.insights.map((ins, i) => (
                        <li key={i} className="flex gap-2 text-sm text-soft leading-relaxed">
                          <iconify-icon icon="solar:check-circle-bold" class="text-emerald-bright shrink-0 mt-0.5"></iconify-icon>
                          {ins}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="space-y-2">
                    {analysis.recommendations.map((rec, i) => (
                      <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-white/5 border border-white/10">
                        <div className="w-5 h-5 rounded bg-emerald text-[#04120C] flex items-center justify-center text-[10px] font-bold shrink-0">{i + 1}</div>
              ...
              <div className="space-y-1">
                {trending.slice(0, 6).map((q) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between gap-3 px-2 py-2 rounded-lg hover:bg-white/5 transition-colors text-left"
                  >
                    <div className="min-w-0">
                      <div className="text-sm font-semibold text-foreground">{q.symbol}</div>
                      <div className="text-xs text-soft tabular-nums">{fmt(q.price, { prefix: '₹' })}</div>
                    </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {MOCK_NEWS.map((n, i) => (
            <Card key={i} className="flex flex-col gap-3">
              ...
              <h3 className="text-sm font-semibold text-foreground leading-relaxed text-pretty">{n.title}</h3>
              <div className="text-xs text-soft">{n.source}</div>
            </Card>
          ))}
        </div>
      )}
      ...
      {tab === 'trending' && (
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {trending.map((q) => (
            <Card key={q.symbol} className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <div className="font-semibold text-foreground">{q.symbol}</div>
                <Change value={q.changePct} />
              </div>
              <div className="text-lg font-bold tabular-nums">{fmt(q.price, { prefix: '₹' })}</div>
              <Sparkline data={getSparkline(q.symbol)} up={q.changePct >= 0} width={220} height={40} />
