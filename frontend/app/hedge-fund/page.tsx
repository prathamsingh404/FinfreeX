...
const signalTone = (s: Signal) => (s === 'bullish' ? 'emerald' : s === 'bearish' ? 'coral' : 'amber') as const
const actionTone = (a: Decision['action']) => (a === 'BUY' ? 'emerald' : a === 'SELL' ? 'coral' : 'amber') as const

export default function HedgeFundPage() {
  const quotes = useMemo(() => getAllQuotes(), [])
  const priceOf = (sym: string) => quotes.find((q) => q.symbol === sym)?.price ?? 1000
  ...
  const backtest = useMemo(() => getBacktest(), [])
            ...
            <Card>
              ...
              <label className="block mb-4">
                ...
                <input
                  ...
                  className="w-full rounded-lg bg-white/5 border border-white/10 px-3 py-2 text-sm text-foreground uppercase outline-none focus:border-emerald-bright"
                />
              </label>
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                {PERSONAS.map((p) => (
                  <button
                    ...
                    className={`rounded-lg border px-3 py-2 text-left transition-colors ${personas.includes(p.id) ? 'border-emerald/40 bg-emerald/10' : 'border-white/10 hover:bg-white/5'}`}
                  >
              ...
              <Btn onClick={runAnalysis} disabled={loading} className="w-full justify-center">
              ...
              <div className="space-y-2">
                {ANALYSTS.map((a) => (
                  <div key={a.id} className="flex items-center gap-2 text-sm text-soft">
                    <iconify-icon icon={a.icon} class="text-emerald-bright"></iconify-icon>
                    {a.label}
                  </div>
                ))}
              </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            {!result && !loading && (
              <Card className="grid place-items-center py-16 text-center">
                <iconify-icon icon="solar:buildings-3-bold-duotone" width="40" class="text-emerald-bright mb-3"></iconify-icon>
              ...
              <Card pad={false}>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {decisions.map((d) => (
                        <tr key={d.ticker} className="border-b border-white/5">
              ...
              return (
                <Card key={ticker}>
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    {signals.map((s) => (
                      <div key={s.agent} className="rounded-xl bg-white/5 border border-white/10 p-3">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
                            <iconify-icon icon={s.icon} class="text-emerald-bright"></iconify-icon>
                            {s.agent}
                          </div>
