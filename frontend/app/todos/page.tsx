...
export default async function Page() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground p-8 flex flex-col items-center justify-center font-sans">
      <div className="w-full max-w-md bg-hover border border-border rounded-2xl p-6 shadow-xl backdrop-blur-md">
        ...
        <ul className="space-y-2">
          {todos && todos.length > 0 ? (
            todos.map((todo) => (
              <li key={todo.id} className="p-3 bg-hover border border-border rounded-xl flex items-center justify-between">
        ...
        <div className="mt-6 pt-4 border-t border-border flex justify-between items-center text-[10px] text-muted font-mono">
          <span>Connected</span>
